...
app.get("/api/questions", async (req, res) => {
  ...
  res.json(currentQuestions.map(q => ({ id: q.id, title: q.title })));
});

app.get("/api/question/:id", async (req, res) => {
  const currentQuestions = await getQuestions();
  const question = currentQuestions.find(q => q.id === req.params.id);
...
app.post("/api/questions", async (req, res) => {
  try {
    const newQuestion = req.body;
    // VALIDATION: Matches server.test.js exactly
    if (!newQuestion.id || !newQuestion.title || !newQuestion.description || !newQuestion.boilerplate || !newQuestion.testCases) {
      return res.status(400).json({ error: "Invalid question format. Missing required fields." });
    }
...
app.post("/api/submit", async (req, res) => {
  const { code, language, questionId } = req.body;
  if (!code || !language || !questionId) return res.status(400).json({ error: "Missing fields" });
...
const worker = new Worker(QUEUE_NAME, async (job) => {
  ...
  const tests = [...question.testCases.sample, ...question.testCases.hidden];
  const results = { sampleResults: [], hiddenPassed: 0, hiddenTotal: question.testCases.hidden.length };

  for (const t of tests) {
    const resp = await axios.post(JUDGE0_URL, { 
        source_code: code, 
        language_id: language === "python" ? 71 : 62, 
        stdin: t.input 
    });
    const actual = (resp.data.stdout || "").trim();
    const passed = actual === (t.expectedOutput || "").trim();

    if (t.isSample) results.sampleResults.push({ input: t.input, passed });
    else if (passed) results.hiddenPassed++;
  }
  return results;
}, { connection });
...
// CRITICAL: Must export for testing
module.exports = app;